...
function Detail() {
  ...
  const playTrailer = async (series_id) => {
    try {
      const response = await axios.get(`https://api.themoviedb.org/3/tv/${series_id}/videos?language=en-US&api_key=${API_KEY}`);
                  ...
                  <div className="flex gap-6 mt-12 rounded-lg items-center">
                    ...
                    <div className="w-3/4 flex flex-col justify-between text-white">
                      ...
                      <div className="left-0 p-4 mr-2">
                        <button className="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 flex items-center" onClick={() => playTrailer(data?.id)}>
                          <FaPlay className="mr-2" />
                          Trailer
                        </button>
                      </div>
                      <div className="hover:text-red-700 mt-6">
                        <Link to={"/trending"}>
